	...
	foreach ($listofreferent as $key => $value) {
		...
		if ($qualified && isset($margin)) {
			...
			if ($key == 'project_task' && !empty($object->lines)) {
						...
						if ($element->statut == Propal::STATUS_NOTSIGNED)
							...
							if (!empty($element->linkedObjects['commande'])) {
								foreach ($element->linkedObjects['commande'] as $linkedOrder) {
									if (!isset($TLinkedOrder['HT']))
										$TLinkedOrder['HT'] = $linkedOrder->total_ht;
									else
										$TLinkedOrder['HT'] += $linkedOrder->total_ht;

									if (!isset($TLinkedOrder['TTC']))
										$TLinkedOrder['TTC'] = $linkedOrder->total_ttc;
									else
										$TLinkedOrder['TTC'] += $linkedOrder->total_ttc;

									if (!isset($TLinkedOrder['nbOrder']))
										$TLinkedOrder['nbOrder'] = 1;
									else
										$TLinkedOrder['nbOrder']++;
								}
							}
						}
					}
				...
				if ($key == 'intervention' && !getDolGlobalInt('PROJECT_INCLUDE_INTERVENTION_AMOUNT_IN_PROFIT'))
